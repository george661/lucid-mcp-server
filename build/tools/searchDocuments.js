...
export const searchDocumentsHandler = async ({ keywords }) => {
    try {
            ...
            if (keywords) {
                ...
                if (allDocs.length === 0) {
                    ...
                    return {
                        content: [
                            {
                                type: "text", text: `No documents found for keywords: "${keywords}" However, found ${allDocs.length} total documents in your account:
                                ${allDocs.slice(0, 10).map((d) => `• ${d.title || 'Untitled'} (ID: ${d.documentId}) - ${d.editUrl || d.viewUrl || 'No URL available'}`).join("\n")}${allDocs.length > 10 ? `\n... and ${allDocs.length - 10} more` : ""}`,
                            },
                        ],
                    };
                }
        ...
        const docList = docs.map((d) => `• ${d.title || 'Untitled'} (ID: ${d.documentId}) - ${d.editUrl || d.viewUrl || 'No URL available'}`).join("\n");
